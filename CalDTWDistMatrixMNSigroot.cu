...
void FPS(const int &sample_num, const string &dataFolder, vector<vector<float>> &TimeSeriesData, vector<vector<float>> &sample_res, vector<vector<float>> &dist_matrix);
...
void CalDTWMatrixMN(vector<vector<float>> M, vector<vector<float>> N, vector<vector<float>> check_res, const string &output);
...
vector<float> GetSingleSignalDataWithRootName(int &SignalId, const string &argv_1, const string &sigRootName);

void getTimeSeriesDataWithRootName(const string &dataFolder, vector<vector<float>> &TimeSeriesData, const string &sigRootName);

void ZScoreNormalize(vector<float> &signals);
int main(int argc, char **argv)
{
  // cout << argc << endl;
  if(argc != 9){cout << "usage: ./CalDTWDistMatrixMNSigroot -i_M FolderPath1 -i_N FolderPath2 -sig_root rootName -o Output_file" << endl;
                cout << "-i_M Folder_path1: 'FolderPath' is the folder path of time series, the file in 'FolderPath' must be $sig_root_0.txt, $sig_root_1.txt,...,$sig_root_M.txt. Here, M-1 is the number of time series." << endl;
                cout << "-i_N Folder_path2: 'FolderPath' is the folder path of time series, the file in 'FolderPath' must be $sig_root_0.txt, $sig_root_1.txt,...,$sig_root_N.txt. Here, N-1 is the number of time series." << endl;
                cout << "-sig_root Signal root name: 'sig_root' is the root name of signal file in folder." << endl;
                cout << "-o Output_file: 'Output_file' is used to store the DTW distance matrix[format: txt]." << endl;
                cout << "Program description: calculate the DTW matrix." << endl;
                exit(0);}
    ...
    cout << "Folder_path1: " << argv[2] << endl;
    cout << "Folder_path2: " << argv[4] << endl;
    cout << "sig_root: " << argv[6] << endl;
    cout << "Output_file:" << argv[8] << endl;

    start = clock();
    vector<vector<float>> TimeSeriesData1{};
    vector<vector<float>> TimeSeriesData2{};

    const string dataFolder1 = argv[2];
    const string dataFolder2 = argv[4];

    vector<vector<float> > dist_matrix{};

    getTimeSeriesDataWithRootName(dataFolder1, TimeSeriesData1, argv[6]);
    getTimeSeriesDataWithRootName(dataFolder2, TimeSeriesData2, argv[6]);

    CalDTWMatrixMN(TimeSeriesData1, TimeSeriesData2, dist_matrix, argv[8]);

    finish = clock();

    duration = (double)(finish - start) / CLOCKS_PER_SEC;
    printf( "The run time of calculating: %f seconds\n", duration);
...
void CalDTWMatrixMN(vector<vector<float>> M, vector<vector<float>> N, vector<vector<float>> check_res, const string &output)
{
  gpuCalculatemnDynamicTimeWarping_2048(M,N,check_res);

  const char *FileName = output.data();

  ofstream disfile(FileName, ios::out);
  string temp;
...
void getTimeSeriesDataWithRootName(const string &dataFolder, vector<vector<float>> &TimeSeriesData, const string &sigRootName)
{
    ...
    // string shell_command = "ls " + dataFolder + "/timeSeries_*" + " | wc -l"; // get shell command.
    string shell_command = "find " + dataFolder + " -name " + sigRootName + "_* " + "| wc -l";
    const int file_num = atoi((getCmdRes(shell_command)).c_str()); //get the file number.
  
    // the following loop for getting all time series data.
    for(int i = 0; i < file_num; i++)
    {
      vector<float> single_data = GetSingleSignalDataWithRootName(i, dataFolder, sigRootName);
      TimeSeriesData.push_back(single_data);
    }
...
void FPS(const int &sample_num, const string &dataFolder, vector<vector<float>> &TimeSeriesData, vector<vector<float>> &sample_res, vector<vector<float>> &dist_matrix)
{
  ...
  {
    gpuCalculatemnDynamicTimeWarping_2048(sample_res, TimeSeriesData, dist_matrix);
    vector<float> point_to_set{};
    for(int column_num = 0; column_num < TimeSeriesData.size(); column_num++)
    {
      vector<float> matrix_colum{};
      for(int row_num = 0; row_num < sample_res.size(); row_num++)
      {
        matrix_colum.push_back(dist_matrix[row_num][column_num]);
      }
      float minValue = *min_element(matrix_colum.begin(),matrix_colum.end());
      point_to_set.push_back(minValue);
    }
    int maxPosition = max_element(point_to_set.begin(),point_to_set.end()) - point_to_set.begin();
    sample_res.push_back(TimeSeriesData[maxPosition]);
    vector<vector<float>> temp{{TimeSeriesData[maxPosition]}};
    delete_ele_in_vector(TimeSeriesData,temp);
    dist_matrix.clear();
  }
  cout << sample_res.size() << endl;
}
...
//     cout << "usage: ./FpsCuda -i FolderPath -sn SamplingNumber -o Output_file";
//     exit(0);
//   }
//   else
//   {
